...
.cwe-pv-foot span:last-child{color:#0B73BA;font-weight:700;letter-spacing:.06em}}";
  ...
  function recommend(cat, st) {
    ...
    var freebies = [];
    var wholeHome = core.some(function (k) { return cat.products[k].kind === "core"; });
    if (wholeHome) { (cat.includedWithSystem || []).forEach(function (k) { if (core.indexOf(k) < 0 && addons.indexOf(k) < 0) { freebies.push(k); } addons = addons.filter(function (a) { return a !== k; }); }); }
  ...
  function resultsHtml(cat, st, cfg) {
    ...
    if (!r.items.length) { return h + nextStepHtml(cat, st, cfg); }
  ...
  function nextStepHtml(cat, st, cfg) {
    var h = "<h3>Next step</h3><p>" + esc(cat.copy.nextStep) + "</p><div class=\"cwe-cta no-print\">" +
